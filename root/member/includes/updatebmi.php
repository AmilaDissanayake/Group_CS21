...
if(isset($_GET['bmi_value'])){
    $bmi_val = $_GET['bmi_value'];
    ...
    if($membership_type == 12 || $member_type == 6){
        $bmi_value_list = $row3['bmi_values'];
        $value_holder = json_decode($bmi_value_list);
        $value_holder[$month-1]=$bmi_val;
        $input_list = json_encode($value_holder);

        $query4 = "UPDATE $package_table SET bmi_values='$input_list' WHERE member_id= '".$member_id."'";
    ...
    if ($result1 && $result2 && $result4) {
        $_SESSION['notification'] = "Successfully update the BMI values!";
        header('Location: ../progress.php');
    } else {
        echo die(mysqli_error($conn));
    }
